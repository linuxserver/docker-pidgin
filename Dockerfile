...
# build stage
FROM ghcr.io/linuxserver/baseimage-alpine:3.24 AS plugins
...
RUN \
  ...
  if [ -z ${PIDGIN_VERSION+x} ]; then \
    PIDGIN_VERSION=$(curl -sL "http://dl-cdn.alpinelinux.org/alpine/v3.24/community/x86_64/APKINDEX.tar.gz" | tar -xz -C /tmp \
