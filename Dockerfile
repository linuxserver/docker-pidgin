# build stage
FROM ghcr.io/linuxserver/baseimage-alpine:3.16 as plugins
...
RUN \
  ...
  if [ -z ${PIDGIN_VERSION+x} ]; then \
    PIDGIN_VERSION=$(curl -sL "http://dl-cdn.alpinelinux.org/alpine/v3.16/community/x86_64/APKINDEX.tar.gz" | tar -xz -C /tmp \
